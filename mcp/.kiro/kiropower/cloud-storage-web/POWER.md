---
name: "tcb-cloud-storage-web"
displayName: "TCB Cloud Storage Web"
...
keywords: ["cloudbase", "cloud storage", "web sdk", "file upload", "file download", "云开发", "云存储", "文件上传", "文件下载", "文件管理", "临时链接"]
author: "CloudBase Team"
---

# TCB Cloud Storage Web

## Overview

This power provides comprehensive guidance for using CloudBase cloud storage in web applications via the `@cloudbase/js-sdk` (Web SDK). It covers all aspects of file storage operations including upload, download, temporary URL generation, file management, and security best practices.

Whether you're building file upload features, managing user-generated content, or implementing file sharing capabilities, this power provides the essential patterns and APIs for CloudBase cloud storage operations.
...
**Problem:** CORS errors during file operations
**Cause:** Domain not whitelisted in CloudBase console
**Solution:**
1. Go to CloudBase Console → Environment → Security Sources → Security Domains
2. Add your frontend domain to the whitelist
3. If errors persist, remove and re-add the domain

**Problem:** File upload fails with permission errors
**Cause:** Insufficient storage permissions or authentication issues
**Solution:**
1. Ensure user is properly authenticated
2. Check CloudBase storage permissions in console
3. Verify file size doesn't exceed limits

**Problem:** Temporary URLs not working
**Cause:** URL expired or invalid fileID
**Solution:**
1. Check if URL has expired (respect maxAge setting)
2. Verify fileID is correct and file exists
3. Generate new temporary URL if needed
...
This skill covers all CloudBase cloud storage Web SDK scenarios:

- **File Upload** - Upload files with progress tracking and error handling
- **Temporary URLs** - Generate secure temporary download links
- **File Management** - Delete and download files efficiently
- **Security** - CORS configuration and access control
