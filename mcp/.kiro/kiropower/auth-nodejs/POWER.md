---
name: "tcb-auth-nodejs"
displayName: "TCB Auth Node.js"
...
keywords: ["cloudbase", "auth", "nodejs", "authentication", "云开发", "身份认证", "Node.js", "服务端认证", "用户身份", "自定义登录"]
...
This power provides comprehensive guidance for implementing CloudBase authentication in Node.js environments. It covers server-side authentication patterns, user identity management, custom login ticket generation, and security best practices for CloudBase applications.

Whether you're building CloudBase cloud functions, Node.js services, or backend systems that need to integrate with CloudBase Auth, this power provides the essential patterns and APIs you need.
...
- Frontend Web login / sign-up flows using `@cloudbase/js-sdk` (handle those with the **CloudBase Web Auth** skill instead).
- Direct HTTP auth API integrations (use the **CloudBase HTTP Auth** skill instead).
...
**Problem:** `getUserInfo()` returns empty or undefined values
**Cause:** Function not called within authenticated context
**Solution:**
1. Ensure the cloud function is called by an authenticated user
2. Verify the CloudBase environment is correctly configured
3. Check that the user is properly logged in on the client side

**Problem:** `getEndUserInfo()` throws permission errors
**Cause:** Insufficient permissions or invalid UID
**Solution:**
1. Verify the UID exists and is valid
2. Check CloudBase console permissions
3. Ensure the function has proper access rights

**Problem:** Custom login ticket creation fails
**Cause:** Missing or invalid credentials file
**Solution:**
1. Download the correct `tcb_custom_login.json` from CloudBase console
...
- Reuse auth instance across the application
- Always handle authentication errors gracefully
- Use environment variables for sensitive configuration
- Implement proper logging for security events
- Follow principle of least privilege for user data access
