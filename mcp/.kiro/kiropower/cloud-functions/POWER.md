---
name: "tcb-cloud-functions"
displayName: "TCB Cloud Functions"
...
keywords: ["cloudbase", "cloud functions", "serverless", "nodejs", "deployment", "云开发", "云函数", "无服务器", "部署", "日志", "HTTP访问"]
...
This power provides comprehensive guidance for developing, deploying, and managing CloudBase cloud functions (Node.js serverless functions). It covers runtime selection, deployment strategies, logging and monitoring, function invocation patterns, and HTTP access configuration.

Whether you're building serverless APIs, background processing functions, or event-driven applications, this power provides the essential knowledge and best practices for CloudBase cloud functions development.
...
**Problem:** Function deployment fails with runtime error
**Cause:** Runtime cannot be changed after creation
**Solution:**
1. Delete the existing function
2. Create a new function with the correct runtime
3. Always specify runtime explicitly during creation

**Problem:** Environment variables not working after update
**Cause:** Direct overwrite deleted existing variables
**Solution:**
1. Query current function configuration first
2. Merge new variables with existing ones
3. Update with merged configuration

**Problem:** Function logs not showing detailed information
**Cause:** Using wrong log query method
**Solution:**
1. Use `getFunctionLogs` to get log list with RequestId
2. Use `getFunctionLogDetail` with specific RequestId for details
3. Ensure time range is within 1 day limit
